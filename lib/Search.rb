...
class Search
  ...
  def start_kayak_search(from_code, to_code, from_date, from_time, to_date, to_time, count)
    ...
    url = "/s/apisearch?basicmode=true&oneway=n&origin=#{from_code}&destination=#{to_code}&destcode=&depart_date=#{from_date}&depart_time=#{from_time}&return_date=#{to_date}&return_time=#{to_time}&travelers=#{travelers}&cabin=f&action=doflights&s=p&apimode=1"
    if count
      url += "&c=#{count}"
    end
	url += "&_sid_=#{sid}"
